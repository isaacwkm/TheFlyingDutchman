...
public class TransitionEffect : SCPEffect
{
    public override string Name => "Pattern";
    public Volume volume;
    public DisableHUD hudDisabler;
    private SCPE.BlackBars patternEffect;
    ...
    private void Start()
    {
        ...
        volume.profile.TryGet(typeof(SCPE.Transition), out patternEffect);
        //Much like in the inspector, a parameter has to be overriden first if you want to modify it.
        patternEffect.size.overrideState = true;
        //Initialize a starting value
        patternEffect.size.value = 0f;
    }

    void Update()
    {
        if (!patternEffect) return;
        ...
        {
            patternEffect.size.value = intensityCurveUp.Evaluate(progress);
        }
        else
        {
            patternEffect.size.value = intensityCurveDown.Evaluate(progress);
        }
